...
COLOR_CYAN = "\033[96m"
log = setup_logger("CLIENT", COLOR_CYAN)
...
class RDTClient:
    def __init__(self, server_ip, server_port, filename, segment_size):
        ...
        self.sock.settimeout(
            PACKET_TIMEOUT_SEC
        )  # No congestion control, so we use a fixed timeout for retransmissions
    ...
    def run(self):
        ...
        log.info(f"Starting transfer for '{self.filename}' with Connection ID: {self.conn_id}")
        try:
    ...
    def _state_init(self):
        ...
        try:
            ...
            log.info(f"Saving file to: {out_filepath}")
            self.state = ClientState.SEND_REQUEST
        except IOError as e:
            log.error(f"Opening file for writing: {e}")
    ...
    def _state_send_request(self):
        ...
        log.info(
            f"Sent REQUEST for {self.filename} (Negotiating segment size: {self.segment_size})"
        )
    ...
    def _state_wait_for_data(self):
        ...
        try:
            ...
            log.error(f"Malformed packet received: {e}")
            return

        if conn_id != self.conn_id:
            log.warning(f"Ignored packet with mismatched Connection ID: {conn_id}")
            return

        if msg_type == MsgType.ERROR:
            log.error(f"Server reported an error: {payload.decode('utf-8', errors='replace')}")
            self.state = ClientState.ERROR
            return
        ...
        if msg_type == MsgType.DATA:
            if seq_num == self.expected_seq:
                ...
                if len(payload) < self.segment_size:
                    log.info("Final packet received. Transfer complete.")
